...
##
## RULES
##   FALL  — water goes down (exclusive, instant; vertical water/air swaps
##           are also how bubbles rise).
##   POUR  — water tips over a lip into dry space (instant, gated).
##   CREEP — puddle edges advance into dry space (half-difference, gated).
##   SEEK LEVEL — each horizontal run of water is one incompressible
##           conduit: columns trade volume surface-to-surface until their
##           levels match, at PRESSURE_RATE per run row (= per square of
##           opening). Non-adjacent columns in a run are fair game: a full
##           capped column between them is a rigid pipe, and through-flow
##           leaves it unchanged.
##
## INVARIANT — every move is downhill or level in elevation (the one rise,
## into a receiver's headroom, requires a full tile of donor head advantage,
## so it is net-downhill too). Potential = Σ(water × height) never increases,
## therefore oscillation and uphill creep are impossible by construction.
##
## AIR is gated, not simulated. A pocket may take water only if its air can
## escape by (a) sky, (b) bubbling up through water above the pocket, or
## (c) expanding into the donor column's own receding headspace.
##
...
var _regions: Array = []      # [{ is_top: bool, vented: bool }]
...
func is_air_sealed(x: int, y: int) -> bool:
	## Debug/renderer helper: the air in this cell cannot reach the sky.
	if not in_bounds(x, y):
		return false
...
func _flood_region(start: int) -> void:
	## Label a connected air pocket, and record where it touches each water
	## body — the highest contact per body. That's the upper junction of a
	## potential rotation: air out high while water comes in low.
...
func _compute_escape() -> void:
	## Mark every cell whose air can reach open sky. Air moves:
	##   1. freely through air, any direction (within a pocket);
	##   2. straight up through water (bubbling);
	##   3. sideways while pinned under a solid ceiling — a bubble pressed
	##      against stone slides along it toward any opening.
	## Monotone fixpoint (flags only flip false→true), so the result is
...
func _cell_pass_all() -> void:
	# Bottom-up scanline, sweep direction alternating per tick: falling
	# columns descend coherently (each cell vacates before the one above it
	# is processed), and the alternating sweep prevents lateral bias.
...
func _cell_pass(i: int) -> void:
	...
	# 2) POUR — over a lip, into dry space only (leveling with water is
	#    seek-level's job, never this rule's)
	if p.y + 1 < height:
		for k in 2:
...
func _gate(t: int, src: int) -> bool:
	## May water from `src` enter dry cell `t`? Only if the displaced air has
	## somewhere to go:
	##   (a) it can reach open sky (the escape flood);
	##   (c) the donor column's headspace is this same pocket — the donor's
	##       water recedes by exactly the transferred amount;
	##   (d) rotation: this pocket and this body also touch higher up — the
	##       air rides the high junction into cells the body vacates while
	##       its water leaves through this one. Water out low, air in high.
...
func _seek_level_run(x0: int, x1: int, y: int) -> void:
	...
	# lowest head first, highest head first — first workable pair wins
	for ti in cols.size():
		var t: int = cols[ti]
...
func _transfer_level(s: int, t: int, y: int, diff: int, tops: PackedInt32Array) -> int:
	## Move volume from column s's surface into column t's surface so their
	## levels converge. Returns units moved.
	...
	if s_top > 0 and stone.is_solid(s, s_top - 1):
		# Capped by stone — but the column may still recede if air can be
		# admitted sideways: a pocket beside the surface expands into the
		# cells the receding water vacates (the top half of a rotation).
...
func _pocket_vented_for(cell: int, s: int, tops: PackedInt32Array) -> bool:
	## Can the air above the receiver's surface give way?
	if _escape[cell]:
		return true
	var s_top: int = tops[s]
	if s_top > 0:
